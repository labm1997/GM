...
class HomeController @Inject()(
   ...
   */
   ...
   def trancamento() = Action { implicit request: Request[AnyContent] =>
    request.session.get("connected").map { user => 
      Ok(views.html.trancamento)
    }.getOrElse {
      Unauthorized("Iiiiish")
    }
  }

  def retirarMateria(arg: Type) = {
     request.session.get("connected").map { user => 
      Ok(views.html.retira_matricula)
    }.getOrElse {
      Unauthorized("Iiiiish")
    }
  }

  def coordenadorRetiraMateria(arg: Type) = {
    request.session.get("connected").map { user => 
      Ok(views.html.gerenciar_turmas)
    }.getOrElse {
      Unauthorized("Iiiiish")
    }
  }
